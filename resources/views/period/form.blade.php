    ...
    <div class="card-body">
      <form autocomplete="off" method="POST" action="{{$action_url}}" enctype="multipart/form-data">
        ...
        <div class="mb-3 col-md-6">
          ...
          <input type="text" class="form-control" placeholder="Nama Periode" value="{{ isset($period->name) ? $period->name : old('name') }}" name="name">
          @error('name')
        ...
        <div class="mb-3 col-md-2">
          ...
          <input type="date" class="form-control" placeholder="Mulai" value="{{ isset($period->start_at) ? $period->start_at : old('start_at') }}" name="start_at">
          @error('start_at')
            <div class="invalid-feedback d-block">
              {{ $message }}
            </div>
        ...
        <div class="mb-3 col-md-2">
          ...
          <input type="date" class="form-control" placeholder="Berakhir" value="{{ isset($period->end_at) ? $period->end_at : old('end_at') }}" name="end_at">
          @error('end_at')
            <div class="invalid-feedback d-block">
              {{ $message }}
            </div>
